...
| `android.permission.USE_BIOMETRIC` | Normal install-time permission | Ask Android to verify an enrolled biometric before releasing an existing owner-bound local credential | Only after the user explicitly enables or invokes biometric unlock; the APP requests biometric-only authentication | Unsupported, unenrolled, failed, or cancelled checks leave the credential locked and return to full server login | Android profiles where the user opts into local biometric unlock |
| `android.permission.USE_FINGERPRINT` | Normal install-time compatibility permission contributed by locked `local_auth_android` 2.0.9 | Support the same biometric-only local credential gate on older Android API levels; it grants no server session authority | Merged from the plugin manifest; invoked only by the explicit biometric unlock flow | Missing or denied compatibility support leaves the credential locked and returns to full server login | Older supported Android devices using the local_auth compatibility path |
| `com.example.rims_frontend.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` | App-defined signature permission generated by AndroidX manifest merging | Restrict AndroidX dynamically registered non-exported receiver traffic to same-signature application components; it grants no RIMS data access | Generated and consumed by the release merged manifest; no runtime prompt | Android signature enforcement denies callers that are not signed with the application certificate | All Android profiles using the AndroidX receiver compatibility path |

The effective merged Android permission baseline is `INTERNET`,
`ACCESS_NETWORK_STATE` from locked `connectivity_plus` 7.1.1, `CAMERA`,
`USE_BIOMETRIC`, compatibility `USE_FINGERPRINT` from locked
`local_auth_android` 2.0.9, and the app-signature
...
| `sdk.local_auth` | local_auth | local_auth 3.0.2 | BSD-3-Clause | OS biometric capability checks and user authentication behind an injected adapter | No TOTP secret, recovery code, password, or server credential is passed to the plugin | Device OS authentication only; success may release an existing local credential but cannot create or extend a server session |
