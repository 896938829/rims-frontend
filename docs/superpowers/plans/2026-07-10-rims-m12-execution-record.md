...
| Boundary | Observed base behavior | M12 disposition |
| --- | --- | --- |
| Runtime permissions | Main manifest declarations are `android.permission.INTERNET` and `android.permission.CAMERA`. The focused architecture test runs `android/gradlew.bat :app:processReleaseMainManifest --no-daemon --console=plain` and requires the sole release main artifact at the current AGP path category `build/app/intermediates/merged_manifest/release/processReleaseMainManifest/AndroidManifest.xml`. Its observed effective merged permissions are `android.permission.INTERNET`, `android.permission.CAMERA`, `android.permission.ACCESS_NETWORK_STATE`, and `com.example.rims_frontend.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`; camera hardware is optional. | OBSERVED local release merged-manifest evidence; purpose and denial rules are recorded in the classification registry |
